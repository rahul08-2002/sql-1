...
alter table customer_income modify customer_id varchar(55);
alter table customer_income add primary key (customer_id);
...
end as grades from customer_income;

create table customer_income_grades(select * ,
case 
when ApplicantIncome >15000 then "grade a"
when ApplicantIncome >9000 then "grade b"
when ApplicantIncome >5000 then "middle class customer"
...
select * from customer_income_grades;

--- criteria_2

create table customer_interest (select * ,
case 
...
end as interest from customer_income_grades);
...
create table loan_status (Loan_ID varchar(55),Customer_id varchar(55), LoanAmount varchar(55),Loan_Amount_Term int,Cibil_Score int);
delimiter //
...
delimiter ;
show triggers;
insert into loan_status values ('LP001002',	'IP43001',	Null,	360,	303),
...
create table remark_updates(loan_id varchar(55),loanamount varchar(55),cibil_score varchar(55),cibil_score_status varchar(55));

delimiter //
create trigger cibil_scores after insert on loan_status for each row
begin
if new.cibil_score>900 then insert into remark_updates(loan_id,loanamount,cibil_score,cibil_score_status) values
(new.loan_id,new.loanamount,new.cibil_score,"High_Cibil_Score");
elseif new.cibil_score>750 then insert into remark_updates (loan_id,loanamount,cibil_score,cibil_score_status)values
(new.loan_id,new.loanamount,new.cibil_score,"NO_Penalty");
elseif new.cibil_score>250 then insert into remark_updates(loan_id,loanamount,cibil_score,cibil_score_status) values
(new.loan_id,new.loanamount,new.cibil_score,"Penalty customers");
elseif new.cibil_score<=0 then insert into remark_updates (loan_id,loanamount,cibil_score,cibil_score_status)values
(new.loan_id,new.loanamount,new.cibil_score,"reject customers");
end if ;
end //
delimiter ;
select * from remark_updates;
drop trigger project1.cibil_scores;
show triggers;
select*from Remark;
Create table Loan_Cibil(select L.loan_id,L.customer_id,L.loanAmount,L.loan_amount_term,L.cibil_score,C.cibil_score_status from loan_status L join Remark C on L.loan_id=C.loan_id);
select*from Loan_cibil;

delete from Loan_cibil where trim(loanamount) = "loan still processing";
delete from Loan_cibil where trim(cibil_score_status)="reject customers";
select*from customer_interest;

------ Month interest and Anuual interest
create table month_interest(select L.Loan_ID,L.Customer_ID,L.ApplicantIncome,L.CoapplicantIncome,L.Loan_status,L.grades,L.interest,C.loanAmount,C.loan_amount_term,C.cibil_score,C.cibil_score_status, 
round ((interest/100)*loanAmount,0)as monthly_interest , round((interest/100)*loanAmount,0) as Annual_interest from customer_interest L join Loan_cibil C on L.loan_id = C.loan_id);
select*from month_interest;

----- sheet 3
select*from customer_det;
update customer_det set gender = 'female' where customer_id = 'IP43006';
update customer_det set gender = 'female' where customer_id = 'IP43016';
update customer_det set gender = 'male' where customer_id = 'IP43018';
update customer_det set gender = 'male' where customer_id = 'IP43038';
update customer_det set gender = 'female' where customer_id = 'IP43508';
update customer_det set gender = 'female' where customer_id = 'IP43577';
update customer_det set gender = 'female' where customer_id = 'IP43589';
update customer_det set gender = 'female' where customer_id = 'IP43593';
...
create table country_region(select C.customer_id,C.Loan_id,C.Customer_name,C.Postal_code,C.Segment,C.State,R.Region,C.Region_id from country_state C left join region_info R on C.Region_id=R.Region_id)
 union 
(select C.customer_id,C.Loan_id,C.Customer_name,C.Postal_code,C.Segment,C.State,R.Region,C.Region_id from country_state C right join region_info R on C.Region_id =R.Region_id);
select*from country_region;
------ output 2
create table output_2(select*from country_region where Region_id is Null);
select*from output_2;

------- Output 3
create table output_3(select*from month_interest where cibil_score=(select max(cibil_score)from month_interest));
select*from output_3;
------ Output 4
create table output_4(select* from country_region where Segment in( "Home Office","Corporate"));
...
create table Output_1(select L.Loan_ID,L.Customer_Id,L.ApplicantIncome,L.Loan_status,L.Grades,L.Interest,L.loanAmount,L.loan_amount_term,L.cibil_score,L.cibil_score_status,L.monthly_interest,
L.Annual_interest,D.Customer_name,D.Gender,D.Age,D.Married,D.Education,D.Self_employed,D.Region_id,C.Postal_code,C.Segment,C.State,C.Region from month_interest L join
 customer_det D on L.customer_id = D.customer_id join country_region C on C.customer_id = D.customer_id);
